...
//            // 선택형(일상)
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(COLLEGE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("고3으로 돌아가서 수능 다시 보기", "지금 이대로 살기")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(COLLEGE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("대학원생 되기", "대학교 7년 다니기")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(COLLEGE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("1교시 수업인데 3연강", "2과목 강의인데 5시간 우주공강")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(COLLEGE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("이른 아침 수업 듣고 낮잠 자기", "밤샘 공부 후 낮 자유시간")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(COLLEGE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("이번 학기 과탑 확정", "오늘 바로 종강하기")).build());
//
//
//
//            // 직장인
...
//            // 선택형(일상)
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(OFFICE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("모두가 천재인 팀에서 숨쉬듯 자괴감 느끼기", "나만이 유일한 희망인 팀에서 소처럼 일하기")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(OFFICE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("불평불만 많지만 마당발인 동료", "조용하고 차분하나 아싸 기질의 동료")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(OFFICE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("안주는 김뿐인 절친과의 술자리", "초호화 고급 안주 무한리필 전체 회식 자리")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(OFFICE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("일은 잘하지만 인성에 문제 있는 동료", "일을 못하지만 참 착한 동료")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(OFFICE).emotion(null).questionType(OPTIONAL).content("밸런스 게임").answerOptions(Arrays.asList("처음부터 끝까지 하나하나 터치하는 사수", "방목시키는 사수")).build());
//
//
//
//            // 기타
...
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(ETC).emotion(null).questionType(SHORT_ANSWER).content("오늘 자기 전 뭐 할 예정이야?").build());
//            // 선택형(일상)
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(ETC).emotion(null).questionType(OPTIONAL).content("오늘 하루 중 얼마나 물을 마셨어?").answerOptions(Arrays.asList("0잔", "1~2잔", "3~4잔", "5잔 이상")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(ETC).emotion(null).questionType(OPTIONAL).content("오늘 하루 중 얼마나 커피를 마셨어? ").answerOptions(Arrays.asList("0잔", "1잔", "2잔", "3잔 이상")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(ETC).emotion(null).questionType(OPTIONAL).content("오늘 얼마나 운동을 했어?").answerOptions(Arrays.asList("30분 이하", "30분 ~ 1시간", "1시간 ~ 1시간 30분", "1시간 30분 이상")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(ETC).emotion(null).questionType(OPTIONAL).content("오늘 얼마나 휴대폰을 했어?").answerOptions(Arrays.asList("1시간 이하", "1~2시간", "2~3시간", "3시간 이상")).build());
//            customQuestionRepository.save(CustomQuestion.builder().userStatus(ETC).emotion(null).questionType(OPTIONAL).content("오늘 받은 스트레스 정도는?").answerOptions(Arrays.asList("1", "2", "3", "4", "5")).build());
//        };
//    }
//}