...
public class DataInitializer {
    ...
    CommandLineRunner initDatabase() {
        return args -> {
            ...
            // 선택형(일상)
            customQuestionRepository.save(CustomQuestion.builder().userStatus(SCHOOL).emotion(null).questionType(OPTIONAL).content("오늘 휴대폰을 얼마나 사용했어?").answerOptions(Arrays.asList("1시간 이하", "1~3시간", "3시간 이상")).build());
            customQuestionRepository.save(CustomQuestion.builder().userStatus(SCHOOL).emotion(null).questionType(OPTIONAL).content("오늘 가장 기분 좋았던 시간은 언제야?").answerOptions(Arrays.asList("오전", "오후", "밤")).build());
            customQuestionRepository.save(CustomQuestion.builder().userStatus(SCHOOL).emotion(null).questionType(OPTIONAL).content("오늘 몇 끼를 먹었어?").answerOptions(Arrays.asList("1끼", "2끼", "3끼 이상")).build());
            customQuestionRepository.save(CustomQuestion.builder().userStatus(SCHOOL).emotion(null).questionType(OPTIONAL).content("오늘 학교에서 가장 재미있었던 수업은 뭐였어?").answerOptions(Arrays.asList("국어", "영어", "수학")).build());
            customQuestionRepository.save(CustomQuestion.builder().userStatus(SCHOOL).emotion(null).questionType(OPTIONAL).content("오늘 스트레스 정도는 어땠어?").answerOptions(Arrays.asList("1", "2", "3")).build());
